...
export default function SubCategoryInfoTable({ handleOpenModal, shop_id, refreshKey }) {
  ...
  const handleDeleteItem = async (id) => {
    try {
      ...
      toast.success('Sub Category successfully deleted from store!', { position: 'top-center' });
  ...
  return (
    <Paper sx={{ width: '100%', overflow: 'hidden', position: 'relative', mb: 2 }}>
            ...
            {rows
              ...
              ?.map((row, i) => (
                <TableRow key={row._id || i}>
                  ...
                  <TableCell>{row?.sub_category_info?.sub_category_name}</TableCell>
