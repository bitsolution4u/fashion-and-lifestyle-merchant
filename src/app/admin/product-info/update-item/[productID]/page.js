...
const UpdateGroceryItems = ({ handleClose, product, index }) => {
  ...
  const handleProductDataSubmit = async () => {
    ...
    productsData.sub_category_info = normalizeId(
      productsData.sub_category_info
    );
    productsData.brand_info =
      normalizeId(productsData.brand_info) || "303030303030303030303030";
    ...
    if (productsData.web_image?.path || productsData.web_image?.relativePath) {
      isImageUpdated = true;
    }

    for (const img of productsData.detail_product_image || []) {
      if (img?.file) {
        isImageUpdated = true;
        break;
      }
    ...
    const typeToUrlMap = {
      ...
      "6880c25e098c458993b3267d": "/api/v1/bag-and-accessories-product/update",
      "6880c282098c458993b32683": "/api/v1/home-decor-product/update",
    };
